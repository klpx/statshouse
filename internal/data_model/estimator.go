...
func (e *Estimator) ReportHourCardinality(time uint32, usedMetrics map[int32]struct{}, builtInStat *map[Key]*MultiItem, aggregatorHost int32, shardKey int32, replicaKey int32, numShards int) {
	...
	for k := range usedMetrics {
		...
		MapKeyItemMultiItem(builtInStat, AggKey((time/60)*60, format.BuiltinMetricIDAggHourCardinality, [format.MaxTagsNew]int32{0, 0, 0, 0, k}, aggregatorHost, shardKey, replicaKey), AggregatorStringTopCapacity, nil).Tail.AddValueCounterHost(cardinality, 1, aggregatorHost)
	}
}
