...
CREATE INDEX IF NOT EXISTS metrics_id_v4 ON metrics_v4 (version);
CREATE INDEX IF NOT EXISTS metrics_id_v4 ON metrics_v4 (type);
...
func OpenDB(
	...
	if opt.Migration {
		err = db.eng.Do(context.Background(), "migration", func(conn sqlite.Conn, bytes []byte) ([]byte, error) {
			rows := conn.Query("check_migration", "SELECT name from property where name = 'migration_v4'")
			if rows.Next() {
				return nil, nil
			}
			if rows.Error() != nil {
			...
			_, err = conn.Exec("finish_migration", "INSERT INTO property (name, data) VALUES ('migration_v4', '')")
			return nil, err
		})
		if err != nil {
			log.Panic(err)
