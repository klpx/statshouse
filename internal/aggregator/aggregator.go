...
func (a *Aggregator) agentBeforeFlushBucketFunc(_ *agent.Agent, now time.Time) {
	...
	writeWaiting := func(metricID int32, key4 int32, item *data_model.ItemValue) {
		key := data_model.AggKey(0, metricID, [format.MaxTagsNew]int32{0, 0, 0, 0, key4}, a.aggregatorHost, a.shardKey, a.replicaKey)
	...
	key := data_model.AggKey(0, format.BuiltinMetricIDAggActiveSenders, [format.MaxTagsNew]int32{0, 0, 0, 0, format.TagValueIDConveyorRecent}, a.aggregatorHost, a.shardKey, a.replicaKey)
	a.sh2.AddValueCounterHost(key, float64(recentSenders), 1, a.aggregatorHost)
	key = data_model.AggKey(0, format.BuiltinMetricIDAggActiveSenders, [format.MaxTagsNew]int32{0, 0, 0, 0, format.TagValueIDConveyorHistoric}, a.aggregatorHost, a.shardKey, a.replicaKey)
	...
	for aggBucket := range a.bucketsToSend {
		...
		for willInsertHistoric && len(aggBuckets) < 1+maxHistoricInsertBatch {
			...
			for _, b := range staleBuckets {
				...
				b.mu.Unlock()
				key := data_model.AggKey(0, format.BuiltinMetricIDTimingErrors, [format.MaxTagsNew]int32{0, format.TagValueIDTimingLongWindowThrownAggregatorLater}, a.aggregatorHost, a.shardKey, a.replicaKey)
